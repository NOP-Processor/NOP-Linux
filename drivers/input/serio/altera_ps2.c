...
static int altera_ps2_write(struct serio *io, unsigned char val)
{
	...
	// printk(" === <ps2 write> === base = %x, val = %x\n", (unsigned int) ps2if->base , (unsigned int)val);
...
static int altera_ps2_open(struct serio *io)
{
	...
	// printk(" <ps2> ecfg = %08x\n", ecfg);
	ecfg |= 0x80;
	// printk(" <ps2> ecfg to be set as  = %08x\n", ecfg);
	write_csr_ecfg(ecfg);
	// ecfg = read_csr_ecfg();
	// printk(" <ps2> ecfg set to %08x\n", ecfg);
...
static int altera_ps2_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "base %x, irq %d\n", ps2if->base, irq);
